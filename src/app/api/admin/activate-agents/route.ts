...
/**
 * POST /api/admin/activate-agents
 * Seed route — insere agentes iniciais no banco.
 * Protegido por requireAdmin (só admin pode acessar).
 * Pode ser removido após o seed inicial.
 */
...
Pergunte sobre o produto, a sequência desejada e o tom de voz antes de começar.`,
    ...
    for (const agent of agents) {
        ...
        if (error) {
            ...
            results.push({ name: data.name, status: 'ok', id: data.id });
        }
    }
